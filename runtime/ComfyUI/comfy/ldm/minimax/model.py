...
def _video_t_grid(n, origin, time_scale=1.0):
    # Reference frames may be sampled below 24fps while retaining source duration.
    spans = torch.tensor(_video_t_spans(n), dtype=torch.float64) * float(time_scale)
...
def _video_grid(vt, frame, cursor, time_scale=1.0):
    g = torch.empty(vt, frame.shape[0], 3, dtype=torch.float64)
    g[:, :, 0] = _video_t_grid(vt, cursor, time_scale)[:, None]
    g[:, :, 1:] = frame[None]
        ...
        if refs:
            ...
            for blk in refs:
                ...
                if kind == "image":
                    ...
                    segments.append(("ref_img", n))
                    pos.append(_video_grid(vt, r_frame, cursor, float(blk.get("time_scale", 1.0))))
                    img_pos.append(torch.arange(row, row + n))
                    img_update.append(torch.zeros(n, dtype=torch.bool))
                    row += n
...
class MiniMaxH3Model(nn.Module):
    ...
    def _run_blocks(self, h, t_emb, mod_segments, rope_freqs, transformer_options, start=0, end=None):
        """TE-Speed upstream block-loop hook; ordinary execution uses start=0."""
    ...
    def _forward(self, x, timestep, context, transformer_options={}, minimax_payload=None, **kwargs):
        ...
        # Blocks are normally executed exactly as before.  The optional
        # sequence wrapper is a narrow diagnostic extension point: it receives
        # the prepared packed features plus this native executor and may elect
        # not to call it.  In that case no prefetch queue is constructed and no
        # block weights are touched.  Ordinary H3 execution has no wrapper and
        # follows the unchanged native branch below.
        def execute_blocks(block_input):
            patches_replace = transformer_options.get("patches_replace", {})
            blocks_replace = patches_replace.get("dit", {})
            ...
            for i, block in enumerate(self.blocks):
                ...
                if ("double_block", i) in blocks_replace:
                    def block_wrap(args):
                        return {"img": block(args["img"], args["t_emb"], args["mod_segments"], args["rope_freqs"],
                                             transformer_options=args["transformer_options"])}
                    block_input = blocks_replace[("double_block", i)](
                        {"img": block_input, "t_emb": t_emb, "mod_segments": mod_segments, "rope_freqs": rope_freqs,
                         "transformer_options": transformer_options},
                        {"original_block": block_wrap})["img"]
                else:
                    block_input = block(block_input, t_emb, mod_segments, rope_freqs, transformer_options=transformer_options)
            if prefetch_queue is not None:
                comfy.model_prefetch.prefetch_queue_pop(prefetch_queue, device, None)
            return block_input

        # TE-Speed-MiniMaxH3-OSS public node protocol.  This takes priority
        # only when its node installed the upstream block_loop patch; the
        # existing project diagnostic seam remains intact for every other run.
        patches_replace = transformer_options.get("patches_replace", {})
        blocks_replace = patches_replace.get("dit", {})
        ...
        if ("block_loop", 0) in blocks_replace:
            ...
            sequence_wrapper = transformer_options.get("minimax_h3_block_sequence_wrapper")
