...
export H3_DEPLOYMENT_MODE="${H3_DEPLOYMENT_MODE:-auto}"
...
if [[ "$H3_DEPLOYMENT_MODE" == "auto" ]]; then
  if [[ -d /.autodl || -d /.autodl-model/data ]]; then
    H3_DEPLOYMENT_MODE="market"
  else
    H3_DEPLOYMENT_MODE="development"
  fi
  export H3_DEPLOYMENT_MODE
fi
...
if [[ "$H3_DEPLOYMENT_MODE" == "market" ]]; then
  ...
  echo "ERROR: H3_DEPLOYMENT_MODE must be auto, market or development, got: $H3_DEPLOYMENT_MODE" >&2
  exit 1
fi
